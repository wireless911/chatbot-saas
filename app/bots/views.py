...
from config.settings import SUB_ACTION
from utils.context import PathContext
from app.bots.swagger import TrainDoc, PublishDoc,ParseDoc,ChatDoc
    ...
    async def post(self, request: Request):
        async def streaming_fn(response):
            ...
            try:
                ...
                # # reload agent
                # bot = app.botsManager.bots[path_context.bot_id]  # type:BotManager
                # app.botsManager.bots[path_context.bot_id] = await bot.load_agent(model_path=model_path)
...
class ModelPublishViews(HTTPMethodView):
    ...
    async def publish(self, app, msg: Optional[Dict] = None):
        try:
            flag = await app.pub.publish_json(SUB_ACTION, msg)
